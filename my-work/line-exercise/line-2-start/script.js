...
console.log(data);
  ...
  graphGroup.selectAll(".line").data(data).enter()
    ...
    function usaFunction(){
      data[0]=incomingData[0];
      console.log(data);
      graphGroup.selectAll("path").data(data).transition()
        .attr("d", lineMaker)
        .attr("stroke",'blue')
    }

    function chinaFunction(){
      data[0]=incomingData[1];
      console.log(data);
      graphGroup.selectAll("path").data(data).transition()
        .attr("d", lineMaker)
        .attr("stroke",'red')
    ...
    document.getElementById("usa").addEventListener("click", usaFunction);
    document.getElementById("china").addEventListener("click", chinaFunction);
